...
| `SELF_STUDY_REFLECT_TIMEOUT_SEC` | `480` | RPC budget (seconds) for the `self_study.reflect` call. Raised from an initial 115s the same day (2026-09-04) once live: the agent-route reasoning model needs real wall-clock time to write its reasoning before the answer, not just tokens -- a real call completed normally in ~121s once given enough token budget to finish (see PR #2085). Raised again 240 -> 480 (2026-09-05) after two real end-to-end reflect calls against a hundreds-of-concepts snapshot both timed out at 240s -- `SELF_STUDY_REFLECT_LLM_ROUTE` defaults to the `agent` lane, already measured elsewhere in this repo as ~2x slower than the chat lane; if 480s still isn't enough, the real fix is moving this route off `agent`, not another bump. Kept under `self_study.reflect.yaml`'s own 520000ms verb/step timeout on purpose, so this file's own RPC wait times out first -- a clean, catchable "no reflection" result instead of the orchestrator killing the step out from under an in-flight wait. |
