...
**Investigation v2:** When `CONTEXT_EXEC_INVESTIGATION_V2_ENABLED=true`, Hub Agent lane sends neutral `mode=investigation_v2` with `permissions` derived from the selected compute lane (`context_exec_permissions_for_llm_profile`) instead of inferring repo access from prompt keywords. Context-exec returns `InvestigationReportV2` with per-source sections (repo/traces/recall/memory/runtime/health), answer status, limitations, and optional LLM synthesis. Recall/LLM gateway bus readiness is preflighted via Redis NUMSUB before long RPC timeouts; dead consumers surface as `dependency_unavailable` or `partial_grounding` with explicit limitations. Synthesis failure records `synthesis_unavailable` without collapsing the deterministic report. Hub renders section summaries inline when v2 is enabled. Default is `false` (legacy keyword mode inference preserved).
