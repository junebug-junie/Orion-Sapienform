"""Side-effect: put ``services/orion-cortex-orch`` first and drop a foreign ``app`` package."""
...
_OTHER_APP_SERVICES = ("orion-mind", "orion-cortex-exec")
...
def ensure_orion_cortex_orch_app() -> None:
    _purge_app_tree()
    _strip_other_service_paths()
    for p in (_REPO_ROOT, _ORCH_ROOT):
        s = str(p)
        if s in sys.path:
            sys.path.remove(s)
