...
class TestMetacogRouteProfile(unittest.TestCase):
    ...
    def test_metacog_background_route_also_injects_atlas_profile_when_missing(self) -> None:
        """Regression (review caught this live 2026-08-29): the profile pin used to be an
        exact `llm_route == "metacog"` string match, so a caller sent to the new
        `metacog_background` route (reverie.py's `_metacog_route()`, once
        ORION_REVERIE_METACOG_BACKGROUND_ENABLED is on) would silently stop being pinned to
        the deterministic model and fall through to the gateway's generic default profile
        instead. Both routes share circe-worker-2 -- see orion/llm/routes.py's
        METACOG_LLM_ROUTES."""
        mock_bus = MagicMock(spec=OrionBusAsync)
        source = ServiceRef(name="test", node="test", version="1.0")
        corr_id = str(uuid4())
        ...
        try:
            ...
            self.assertEqual(result.status, "success")
            req = llm_chat.await_args.kwargs["req"]
            self.assertEqual(req.route, "metacog_background")
            self.assertEqual(req.profile, "llama3-8b-instruct-q4km-atlas-metacog")
        finally:
            settings.atlas_metacog_profile_name = old_profile
