...
So the answer to (a)/(b)/(c)/intentional is **(c) with an intentional narrowing**:
...
`services/orion-durable-runs/app/admission_runtime.py:344-351` (outbox drain in
`reconcile()`):
...
`durable_resource_events` on 2026-09-21 alone: `run.running` 98, `run.resumed` 33,
`run.waiting_resource` 52, `run.admitted` 30, `run.retrying` 4, `run.completed` 17,
`run.failed` 0. The four `run.retrying` rows all carry `{"node": "harness_turn"}` —
...
- It is a contract decision, not a one-line bug: which of the two diaries is the
  ...
  `_handle_run_state` also refunds/mirrors on `completed` only, so forwarding
  `failed` changes Hub-side behavior for real.
...
Extend the outbox bridge in `admission_runtime.py:reconcile()` so every `run.*`
lifecycle event with a status in `DurableRunStatusV1` is forwarded as a
`DurableRunStateV1` on `orion:durable:run:state`, keeping `entry_id =
f"{event.entry_id}:state"` for idempotency (sql-writer's PK dedup already
handles replays), `node = event.detail.get("node") or "<admission>"`, and
`status = event.event.removeprefix("run.")`. Concretely:

```python
STATE_MIRROR = {"waiting_resource", "admitted", "running", "resumed", "retrying",
                "paused", "completed", "failed", "cancelled"}
status = event.event.removeprefix("run.")
if event.event.startswith("run.") and status in STATE_MIRROR:
    mirror = DurableRunStateV1(entry_id=event.entry_id+":state", run_id=event.run_id,
        workflow="curiosity.investigate", thread_id=event.thread_id,
        node=str(event.detail.get("node") or "admission"), status=status,
        correlation_id=event.correlation_id, generated_at=event.generated_at,
        detail=event.detail)
...
one `DurableRunStateV1` per `run.*` outbox event, not just `run.completed`;
(5) a live check that `substrate_durable_run_state` again shows `running`/`failed`
rows for an admitted run. `workflow` should come from the admission row's request
(`row["request"]["workflow"]`) rather than the literal `"curiosity.investigate"`,
since `self_sense_eval` and `self_study.reflect` now also run through admission
(`8502c5fc7`, `2ff385c17`, `23388668c`, 2026-09-21).
...
`runner.py:491-506`) are a separate decision: they were the surface lane's view
...
## Side finding (live, not the question asked): one run is in a hot failure loop
...
3,232 such rows for this one run in `durable_resource_events` between the 02:47 UTC
container start and 06:53 UTC (one every ~4.6 s).
Cause: the run's LangGraph checkpoint holds
`admission.alternatives == ["agent-burst"]` (decoded from `checkpoint_blobs`,
channel `admission`), while its `durable_admission_runs.request` and
...
`updated_at`/`control`/`terminal` updates), Postgres `log_statement=none`, and
the PR #2284 message says the stored row is not rewritten. Runs created after
06:00 UTC today have matching checkpoint and row (`["agent-burst","chat-burst"]`
both), so new runs are not affected. Recommended: compare only caller-supplied
