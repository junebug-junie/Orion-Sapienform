...
**Investigation v2 (PR1 plumbing):** When `CONTEXT_EXEC_INVESTIGATION_V2_ENABLED=true`, requests use neutral `mode=investigation_v2` with profile-derived permissions (Hub compute lane / agent compat `response_profile`). PR1 returns `InvestigationV2SkeletonV1` without recall/repo/trace sweeps. Default is `false` (legacy keyword mode inference preserved).
