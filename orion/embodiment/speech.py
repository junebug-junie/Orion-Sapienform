...
def build_speech_prompt(perception: WorldPerceptionV1, own_player_id: str) -> str:
    ...
    return (
        ...
        f"Speak to {interlocutor} in character, as a real reply to the latest line above — "
        f"not a dismissive acknowledgment and not a chat-bubble scrap. "
        f"Match substance to what they said: a greeting can stay brief; a question, plan, "
        f"or technical point deserves a clear answer (a few short spoken sentences is fine). "
        f"Stay spoken and in-scene — no essay, no list dump, no stage directions. "
