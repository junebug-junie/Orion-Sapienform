 ...
 *
 * BNO085: I2C is the Adafruit breakout default (PS0/PS1 low → 0x4A).
 * UART-RVC (D6/D7) is a fallback only.
 */
...
// Cached IMU samples (I2C reports arrive as separate events).
...
static void reinit_i2c_bus() {
  // LTR390 soft-reset can leave ESP32 Wire hung; recover before remaining inits.
  Wire.end();
  delay(10);
  init_i2c_bus();
}
...
static void init_pmsa003i() {
  have_pmsa003i = pm25.begin_I2C(&Wire);
}
...
static void emit_boot_diagnostic() {
  ...
  if (!bno_ok) {
    if (i2c_has_addr(0x4A) || i2c_has_addr(0x4B)) {
      bno["detail"] = "i2c_begin_failed";
    } else {
      bno["detail"] = "not_on_bus_or_uart";
    }
  }
...
static void poll_bno_i2c_events() {
  ...
  // Drain a few pending events per tick.
...
void setup() {
  ...
  // Wake / settle PMSA fan before probing the bus.
...
void loop() {
  ...
  // Re-emit boot diagnostic periodically so host can capture without racing setup().
