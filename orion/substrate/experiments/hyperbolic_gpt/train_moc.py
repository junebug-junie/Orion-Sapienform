...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train MoC HyperbolicGPT v3")
    p.add_argument("--dataset", choices=("tinystories", "text"), required=True)
    p.add_argument("--text_path", type=str, default=None)
    p.add_argument("--out_dir", type=str, default="./runs/hyperbolic_gpt_moc")
    ...
    p.add_argument("--untie_lm_head", action="store_true", default=False)
    return p.parse_args()
...
def main() -> None:
    ...
    if rank == 0:
        ...
        summary = {
            ...
            "effective_batch_sequences": args.batch_size * args.grad_accum * world_size,
            "effective_batch_tokens": args.batch_size * args.grad_accum * world_size * args.block_size,
            "args": vars(args),
    ...
    def _optimizer_step() -> None:
        ...
        if opt_step % args.eval_interval == 0:
            ...
            ddp_print(
                ...
                f"cμ {ev_mean['moc_curvature_mean']:.4f} "
                f"cσ {ev_mean['moc_curvature_std']:.4f} "
                f"λμ {ev_mean['moc_geo_lambda_mean']:.4f} "
                f"λσ {ev_mean['moc_geo_lambda_std']:.4f}",
            )
