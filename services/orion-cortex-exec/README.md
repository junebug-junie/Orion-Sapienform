...
| `SELF_STUDY_REFLECT_TIMEOUT_SEC` | `115` | RPC budget (seconds) for the `self_study.reflect` call. Kept under `self_study.reflect.yaml`'s own 130000ms verb/step timeout on purpose, so this file's own RPC wait times out first -- a clean, catchable "no reflection" result instead of the orchestrator killing the step out from under an in-flight wait. |
