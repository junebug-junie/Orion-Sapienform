...
def preprocess_data(data, input_template=None, input_key="input", apply_chat_template=None) -> str:
    if apply_chat_template:
        format_prompt = [{"role": "system", "content": "Please reason step by step, and put your final answer within \\boxed{}."}, {
            "role": "user",
...
def batch_generate_vllm(args):
    ...
    sampling_params = SamplingParams(
        ...
        temperature=args.temperature if not args.greedy_sampling else 0.0,
    ...
    problem_set = "/workspace/chengxing/evals/glm-simple-evals/data/math/math500.json"
    # "/workspace/chengxing/evals/glm-simple-evals/data/math/math500.json"
    with open(problem_set, "r") as f:
        data = json.load(f)
    data = [{"Question": d['problem'], "Answer": d['answer']} for d in data]
    if len(data) // args.max_samples > 0:
        data = data[::len(data) // args.max_samples]
    # problem_set = "/workspace/chengxing/evals/glm-simple-evals/data/gsm8k/gsm8k.json"
    # problem_set = "/workspace/chengxing/evals/glm-simple-evals/data/drop/drop.json"
    # "/workspace/chengxing/evals/glm-simple-evals/data/math/math500.json"
    # "/workspace/chengxing/evals/glm-simple-evals/data/aime/aime.json"
    # with open(problem_set, "r") as f:
    #     data = json.load(f)
    # with open("/workspace/chengxing/evals/glm-simple-evals/data/math/math500.json", "r") as f:
    #     data = json.load(f)
    # prompts_data = [{"Question": d['problem'], "Answer": d['answer']} for d in data][:args.max_samples]
    # data = [{"Question": d['question'], "Answer": d['answer']} for d in data][:args.max_samples]
    prompts_data = [d['Question'] for d in data]#[:2]
    answers_data = [d['Answer'] for d in data]#[:2]
    N = args.best_of_n if not args.greedy_sampling else 1
    prompts_dataset = MathDataset(prompts_data, tokenizer, dummy_strategy, input_template=args.input_template)
    ...
    patten_prompt = f'([\s\S]*){eos_token}'

    outputs = llm.generate(prompts * N, sampling_params)
    for output in outputs:
        prompt = output.prompt
        output = output.outputs[0].text
        matchs = re.findall(patten_prompt, output)
        if matchs is not None and len(matchs) >= 1:
            output = re.findall(patten_prompt, output)[0]
    ...
    data = [
        {
            "id": i,
            # "raw": output_dataset[i]["input"],
            "instruction": prompts_data[i % lens],
            "output": output_dataset[i]["output"],
            "answer": answers_data[i % lens]
        }
    ...
    f2 = open(os.path.join(args.output_path, args.output_name), 'w')
    f2.write(b)
    f2.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_name", type=str, default="output.json", help="Output JSON data path")
