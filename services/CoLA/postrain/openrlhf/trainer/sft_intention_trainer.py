        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for prompts_id_lens, inputs, attention_masks, infos in self.train_dataloader:
                ...
                if self.add_label_noise:
                    random_label = torch.randint_like(inputs, 0, 128000)
                    random_mask = (torch.rand(inputs.shape) < self.label_noise_ratio)
                    inputs[random_mask] = random_label[random_mask]
                    # print("adding noise")
                
                if self.stage == 1:
                    output = self.model(inputs, attention_mask=attention_mask, return_output=True, vq_mode=True)
                    # loss function
                    ...
                    output, labels = self.model(inputs, attention_mask=attention_mask, return_output=True, bc_mode=True)
                    logits = output
                    labels_ = torch.where(
                        attention_mask.bool(),
                        inputs,
                    ...
                    labels[labels_ == self.loss_fn.IGNORE_INDEX] = self.loss_fn.IGNORE_INDEX
                ...
                acc_sample_num = (pre_labels[..., :-1] == labels[..., 1:]).float().sum()
                total_sample_num = (labels[..., 1:] != self.loss_fn.IGNORE_INDEX).float().sum()
                acc_ratio = acc_sample_num / total_sample_num

                loss_mean = loss_mean * 0.9 + 0.1 * gpt_loss.item()
                logs_dict = {
                    "gpt_loss": gpt_loss.item(),
                    "gpt_acc": acc_ratio.item(),
                    "loss_mean": loss_mean,
