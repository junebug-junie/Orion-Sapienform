...
| `orion:gpu_pool:control:request` | `GpuPoolControlV1` (lend, unlend, hold, release, replay, cancel, backfill), **HMAC-signed** (`orion/gpu_pool/control_auth.py`: 60 s freshness, single-use nonce), so the operator secret never rides the bus | operator (Hub) → pool (RPC) |
