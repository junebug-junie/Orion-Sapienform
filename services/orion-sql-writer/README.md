...
**Drive audit persistence — table is frozen/historical as of 2026-07-30:** `orion-spark-concept-induction`'s `DriveEngine` (the sole producer of `memory.drives.audit.v1`) was deleted outright (`chore/delete-orion-drives`, PR #1486), following through on `orion/sentience_striving_program/README.md` §8's halt. This service still subscribes to the channel and would still project any message into `drive_audits` (PK `artifact_id`, idempotent upsert; `active_count` derived at write time as `len(active_drives)`), but none will ever arrive again. `DRIVE_AUDITS_RETENTION_DAYS` was changed from its old default of 90 to **0 (disabled)** as part of the same PR, specifically so this now-finite history isn't silently pruned away — Hub Drives Analytics still reads it, relabeled "(historical)". `scripts/analysis/measure_autonomy_gate.py` (the drive co-activation verdict script, successor to the Fuseki DriveAudit graph frozen since 2026-06-19) can still run against this frozen dataset but will never see new data. DDL is applied on boot (`app/main.py` lifespan) and also lives in `services/orion-sql-db/manual_migration_drive_audits_v1.sql`.
