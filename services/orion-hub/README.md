...
**Memory consolidation graph drafts:** `orion-memory-consolidation` persists `memory_graph_suggest_drafts` (`pending_review`) when conversation windows close. Hub Memory tab → **Graph drafts** lists automated drafts from the same Postgres DSN as memory cards (`RECALL_PG_DSN`). **Load in editor** prefills the graph annotator; **Validate / Approve** uses existing `/api/memory/graph/*` and marks the consolidation draft `approved` when `consolidation_draft_id` is sent on approve. Reject via **Graph drafts** or `POST /api/memory/consolidation/drafts/{draft_id}/status`. Requires `services/orion-sql-db/manual_migration_memory_consolidation_v1.sql` applied on the memory Postgres.
