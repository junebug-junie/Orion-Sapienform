"""Regression coverage for social-memory's bus-native SystemHealthV1 heartbeat wiring.
...
pilot-5 pattern proved out in PR #1350. orion-social-memory had no bus-native heartbeat before
this patch (confirmed via `grep -rl "BaseChassis"` returning nothing for this service).
`service` (the real SocialMemoryService, owning its own bus connection) is mocked out here so
...
import app.main as social_memory_main
from orion.core.bus.bus_service_chassis import HeartbeatOnly


def test_build_heartbeat_chassis_uses_service_settings() -> None:
    chassis = social_memory_main.build_heartbeat_chassis()
    assert isinstance(chassis, HeartbeatOnly)
    assert chassis.cfg.service_name == social_memory_main.settings.service_name
    assert chassis.cfg.service_version == social_memory_main.settings.service_version
    assert chassis.cfg.node_name == social_memory_main.settings.node_name
    assert chassis.cfg.bus_url == social_memory_main.settings.orion_bus_url
    assert chassis.cfg.bus_enabled == social_memory_main.settings.orion_bus_enabled
    assert chassis.cfg.heartbeat_interval_sec == social_memory_main.settings.heartbeat_interval_sec
...
async def test_lifespan_starts_and_stops_heartbeat_chassis() -> None:
    ...
    with patch.object(social_memory_main, "service") as mock_service, patch(
        ...
        app_stub = object()
        async with social_memory_main.lifespan(app_stub):  # type: ignore[arg-type]
            assert social_memory_main.heartbeat_chassis is fake_chassis
            fake_chassis.start_background.assert_awaited_once()

        fake_chassis.stop.assert_awaited_once()
        assert social_memory_main.heartbeat_chassis is None
