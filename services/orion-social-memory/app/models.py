...
class SocialParticipantContinuitySQL(Base):
    ...
    calibration_signals = Column(JSON, nullable=True)
    peer_calibration = Column(JSON, nullable=True)
    trust_boundary = Column(JSON, nullable=True)
    memory_freshness = Column(JSON, nullable=True)
    decay_signals = Column(JSON, nullable=True)
    regrounding_decisions = Column(JSON, nullable=True)
...
class SocialRoomContinuitySQL(Base):
    ...
    gif_usage_state = Column(JSON, nullable=True)
    active_commitments = Column(JSON, nullable=True)
    calibration_signals = Column(JSON, nullable=True)
    peer_calibrations = Column(JSON, nullable=True)
    trust_boundaries = Column(JSON, nullable=True)
    memory_freshness = Column(JSON, nullable=True)
    decay_signals = Column(JSON, nullable=True)
    regrounding_decisions = Column(JSON, nullable=True)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
