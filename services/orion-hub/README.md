...
**Read evidence.** A Stage 1 seed is only marked `done` when the harness tool trace shows the turn actually fetched its source: a WebFetch (or firecrawl / context-mode fetch) on the seed's site whose result carried at least 200 characters. The governor derives this from raw tool_use/tool_result pairs (`HarnessRunV1.source_fetches`, `orion/harness/reading_receipts.py`), Hub puts it on the final frame as `harness_source_fetches`, and it is stored on the handoff as `read_evidence`. A turn without it is `failed` with `last_error=no_read_evidence` (terminal, slot charged -- the reader ran); `no_read_evidence:harness_unreported` means the governor predates the field (retried as transient -- deploy the governor before Hub). Stage 2 skips a handoff with no `read_evidence` (`stage2_status=skipped`, `stage2_error=no_read_evidence`) before debiting Wallet B. Policy: `orion/world_pulse_read/read_evidence.py`. Log line: `world_pulse_read_fetch_evidence`.

**Stale digest items.** Every Stage 1 tick marks pending `digest_item` seeds older than `HUB_WORLD_PULSE_READ_DIGEST_ITEM_MAX_AGE_DAYS` (default 5, 0 disables) as `skipped` / `stale_digest_item`. Digest items are priority 10 behind findings and readings, so without this the tail only grew older. Log line: `world_pulse_read_skipped_stale_digest_items`.
