...
def test_tokenless_atomic_drain_and_resume():
    client=TestClient(m.app)
    assert client.post('/v1/lifecycle/drain',json={"draining":True}).json()["draining"]
    ...
    assert not client.post('/v1/lifecycle/drain',json={"draining":False}).json()["draining"]
...
def test_cancelled_waiter_retains_real_gpu_occupancy(monkeypatch):
    ...
    async def scenario():
        ...
        await m.drain(m.DrainRequest(draining=True))
