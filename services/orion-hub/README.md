...
**Wallet refunds.** Both reading loops debit their wallet (daily count + cooldown) right after claiming a seed, before the turn runs. A turn that stops before any reading happened -- the stance phase deferred or refused it (`turn_deferred:*`, which is where a GPU capacity refusal such as `gpu_pool_unavailable` lands) or there was no bus (`bus_unavailable`) -- gets its daily slot back on the day it was charged, and its cooldown shortened to the configured floor (`HUB_WORLD_PULSE_READ_MIN_COOLDOWN_SEC` / `..._STAGE2_MIN_COOLDOWN_SEC`) instead of the full paced spacing. Turns that reached the reader (`turn_error:*`, timeouts, parse failures) keep their charge. The seed itself still spends one of its `max_attempts`. Log lines: `world_pulse_read_wallet_refunded` / `world_pulse_read_stage2_wallet_refunded`. Policy: `orion/world_pulse_read/retry.py::is_refused_before_work`; mechanics: `orion/world_pulse_read/wallet_refund.py`.
