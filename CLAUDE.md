...
4. **Live-data sanity check.** Pull real data and look at it. Confirm it is not degenerate (flat, always-null, always-saturated) before treating it as usable signal. Config existing or code compiling is not evidence. This is not just "does it vary" — check whether it can ever return to a genuine rest/calm state, not merely whether the numbers move. A metric can vary continuously and still be structurally incapable of reading "calm": `bus_synaptic_prediction_error()` (2026-07-26) varied in real time yet had a mathematically permanent ~0.27 floor (`mean(|z|)` for a calm z-score population has expected value `sqrt(2/pi)`, not 0) — caught only by recovering the underlying pre-aggregation statistic from the already-stored value and checking its theoretical rest point by hand, not by eyeballing the aggregate's variance. Conversely, a metric reading a suspiciously clean 0.0 is not automatically "confirmed calm" either — `node:substrate.route`'s `prediction_error` (same day) went subnormal and was heading to exact 0.0 because a generic staleness-decay loop (`services/orion-field-digester/app/digestion/decay.py`'s `NODE_DECAY_CHANNELS`) was silently multiplying it by 0.92 every tick for 48+ hours since nothing ever refreshed it — a *decayed-to-zero* artifact that looked identical to *genuinely-calm-at-zero* until the raw history was pulled and the exact geometric ratio between successive values was checked by hand. Both directions of this failure were only found by recovering the real numbers from what was already persisted, not by re-deriving from code alone.
...
**Uncommitted worktree work is not unprotected, but it is not committed either — commit real work yourself, don't rely on this.** A `Stop` hook (`scripts/hooks/stop_worktree_wip_snapshot.py`, throttled to once per 5 minutes across every concurrent session, and itself returning near-instantly by handing the actual scan to a detached background process) snapshots every dirty worktree's current content — tracked and untracked, respecting `.gitignore` — into a hidden `refs/orion-wip/<branch>/latest` ref via `write-tree`/`commit-tree` plumbing. This never touches the real index, working tree, HEAD, or the branch itself — it cannot appear in `git log`, a PR diff, or interrupt a half-finished edit, because it only fires at a genuine turn boundary (Stop), never on a mid-session timer. Recover from one with `git -C <worktree> show refs/orion-wip/<branch>/latest:<path>` (one file) or `git -C <worktree> checkout refs/orion-wip/<branch>/latest -- .` (the whole snapshot, staged on top of the current working tree — verified live: this is a PLAIN commit, not stash-shaped, so `git stash apply` on it fails with "not a stash-like commit"; `git checkout <commit> -- .` is the form that actually works on any commit). This exists purely as a last-resort safety net after a real near-miss (2026-08-18: ~1,000 lines of real work sitting uncommitted in an already-merged-branch worktree, discovered only right before a main pull/deploy) — it is not a substitute for committing and pushing real work as you go.
...
Manual `checkin`/`heartbeat`/`add`/`checkout` calls above are still the right tool for anything with real content (a finding, a blocker, an explicit task handoff). But a passive reminder to run them by hand does not reliably work: a live session read a stop-hook banner saying "no open board items are recorded for this worktree" across roughly 20 consecutive turns and never acted on it until asked directly, because there was no cost to ignoring it. Two automatic, best-effort heartbeats now ride on habits that are already universal instead of depending on an agent noticing a banner:

- **`scripts/git_hooks/post-commit`** (installed by `scripts/install_git_safety_hooks.sh`) heartbeats the board with the commit's own subject line after every commit, in every worktree. Because `post-commit` is a hook name real tooling in this repo already uses (`graphify hook install`'s knowledge-graph rebuild hook), the installer appends its block to any existing foreign `post-commit` hook rather than overwriting it the way `pre-commit`/`post-merge` do — confirmed live: a naive overwrite silently disabled graphify's rebuild-on-commit the first time this shipped. Each fragment is wrapped in its own subshell (`( ... ) || true`), since a plain `exit` in either script would otherwise terminate the whole concatenated file — also confirmed live, since graphify's own hook calls `exit 0` for any commit made from a linked worktree. `install_git_safety_hooks.sh` is safe to re-run: its own block is bounded by explicit BEGIN/END markers, so a refresh finds both ends of it and preserves whatever content sits on either side — including content another tool's installer appended after ours on a later run.
- **`scripts/safe_docker_build.sh`** heartbeats before every docker compose invocation it wraps (build, up, config, logs — the wrapper has no reliable way to distinguish read-only from mutating compose args, and announcing a read-only check is cheap), naming the service and task as `deploy:<service>`. This is the collision surface with the most direct incident history (section 8's shared-checkout deploy story) and the most direct match to a live report of concurrent agents hitting real collisions on similar topics.
...
- A `pre-commit` gate now backstops this: if `graphify-out/graph.json` is staged and its node count dropped more than 10% versus HEAD, the commit is refused (`scripts/check_graph_node_loss.py`, installed by `scripts/install_git_safety_hooks.sh`). The wrapper guards the *command*; this guards the *artifact*, so it fires however the damage happened -- a bare `graphify update`, another tool, a human's terminal, an agent not running under Claude Code, or a bypassed wrapper. That matters because the 2026-07-14 incident was committed and merged to main before anyone caught it, and as of 2026-08-14 the destructive update still recurs several times a day. Threshold override: `GRAPHIFY_COMMIT_MAX_NODE_LOSS_PCT`. Escape hatch for a genuine full re-extraction, set consciously per command: `ORION_ALLOW_GRAPH_SHRINK=1 git commit ...`. Verified against the real 43MB artifact: gutting it 28306 -> 2475 nodes is refused at 91.26% loss, and a commit that does not touch the graph is unaffected (whole pre-commit runs in 0.40s). None of this fixes the underlying graphify bug, which is still unroot-caused -- it contains the damage.
- After modifying code, run `scripts/safe_graphify_update.sh` (NOT `graphify update .` directly) to keep the graph current (AST-only, no API cost). A real incident (2026-07-14) found `graphify update .` can silently shrink the graph by ~95% while its own log output claims to have "kept" the old nodes -- that got committed and merged to main before being caught. The wrapper compares node count before/after and auto-restores if the drop exceeds a threshold (default 10%, override with `GRAPHIFY_UPDATE_MAX_NODE_LOSS_PCT`), so a destructive run produces nothing to commit instead of silent data loss. **ROOT-CAUSED 2026-09-06 (PR: fix/graphify-scan-scope).** graphify 0.9.15 applies a nested `.gitignore`'s *unanchored* patterns to the entire remaining directory walk, not just that file's subtree. `services/orion-hub/tests/e2e/artifacts/.gitignore` held a bare `*`, so every directory os.walk visited after it was pruned, the scanner reported **0 code files at repo root** (4,433 once fixed), and the incremental path's AST-ownership rule evicted every code node it no longer 'owned' -- while its own log said "kept 26,564 node(s)". The ignore is now the anchored `/*` form (same meaning to git), `tests/test_gitignore_no_bare_catchall.py` refuses any tracked bare catch-all, and `scripts/check_graphify_scan_scope.py` (run by the wrapper before every refresh) refuses a hollow scan outright instead of catching the wipe after the fact. First clean refresh: 28,306 -> 74,860 nodes. If the wrapper still refuses, read its message -- it now names which layer tripped.
...
- **`graphify-out/graph.json` is git-LFS-tracked as of 2026-09-08** (`fix/graph-json-lfs`, forward-only -- existing history NOT rewritten): a routine refresh pushed it past GitHub's hard 100MB-per-blob limit. A fresh clone/worktree needs `git lfs install` run once per machine, or this file checks out as a ~130-byte pointer stub instead of real content -- check whether it's already global with `git config --global --get filter.lfs.smudge` before assuming you need to run it. The merge driver above now goes through `scripts/graphify_lfs_merge_driver.sh`, not `graphify merge-driver` directly, because git feeds a merge driver raw (unsmudged) LFS pointer stubs. **Known open risk, accepted, not solved:** this repo is public on GitHub's free LFS tier (1GB storage + 1GB/month bandwidth, account-wide) -- check consumption with `git lfs env` or GitHub's repo Settings -> Billing -> Git LFS Data page before assuming headroom.
