...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("image/webp", ".webp"),
        ("image/gif", ".gif"),
        ("image/tiff", ".img"),      # unknown mime, no usable filename suffix
    ],
)
def test_suffix_comes_from_mime(store, sandbox, mime, expected):
    out = stage_attachments(
        [ref(mime=mime, filename=None)], correlation_id="c", source_dir=store, root=sandbox
    )
...
def test_a_hostile_correlation_id_cannot_escape_the_sandbox(sandbox):
    d = staging_dir_for("../../../etc/evil", root=sandbox)
    assert d.is_relative_to(sandbox)
