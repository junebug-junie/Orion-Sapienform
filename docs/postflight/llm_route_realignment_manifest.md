# LLM Route Realignment Manifest (Merged Default + FAST quick lane)

## Scope

This change set keeps logical route contracts intact (`chat`, `agent`, `metacog`, `quick`) while changing default Atlas physical placement:
...
- `quick` (Hub-visible fast lane) → `atlas-worker-fast-1` (`8013`)
...
- `chat_general` remains two-pass:
  1) `synthesize_chat_stance_brief` on `route="quick"`
