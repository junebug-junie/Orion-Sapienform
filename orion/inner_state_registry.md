...
Relocated from `orion/self_state/README.md` (2026-07-22, SelfStateV1 burn) --
this documentation is about the registry itself, which tracks every
"what does Orion currently feel/perceive" signal in the repo, not just
`SelfStateV1`'s own composition. It survives the burn; `orion/self_state/`
(the module that used to house it) does not.

`SelfStateV1` was the one schema every cognition-facing prompt-builder
(metacog templates, chat prompts) was expected to read from -- directly, or
...
ones `SelfStateV1` itself composed: `FieldStateV1`, `FieldAttentionFrameV1`,
...
- `COMPOSED` -- has a named field on `SelfStateV1` (or, for phi, is wired
  ...
  prompt templates -- confirmed live in production before this phase shipped
...
- `REHEARSAL` -- computed, verified to reach no cognition consumer at all.
  ...
  `FIELD_CHANNEL_ANOMALY_ENABLED` (default off) and currently pointed at a
  separately-tracked model directory, not this entry's own `v4`. Full detail
  ...
  is already excluded from phi's live trainable feature set -- see
...
dimensions pinned/flat in a live replay). See
`docs/superpowers/specs/2026-07-22-self-state-phi-endo-origination-burn-spec.md`
and `docs/superpowers/specs/2026-07-22-self-state-phi-burn-brainstorm.md`
for the full trace. `orion/self_state/` (builder.py, scoring.py, policy.py,
deviation.py, prediction.py, transport.py), `services/orion-self-state-runtime/`,
and their dedicated tests were deleted as part of that burn; this registry
file was relocated here (from `orion/self_state/inner_state_registry.py`)
rather than deleted with them, since it tracks signals well beyond
`SelfStateV1` and its CI gate has independent value.
