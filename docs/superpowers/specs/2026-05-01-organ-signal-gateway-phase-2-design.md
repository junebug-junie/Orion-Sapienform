...
| **Contradictory test layout** | **Canonical layout:** **`orion/signals/adapters/tests/`** — pure adapter tests (fixtures, `prior_signals` dict injection, no live Redis). **`services/orion-signal-gateway/app/tests/`** — gateway integration (processor, passthrough, OTEL, window, multi-envelope). Cross-import helpers live in `orion/signals/` test utilities only if shared. Update the **File map** section below and phase-1 §8 (Testing) / file map by errata. |
