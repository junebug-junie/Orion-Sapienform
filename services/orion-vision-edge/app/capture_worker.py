...
async def capture_loop():
    ...
    while True:
        ...
        if frame is None:
            none_frame_count += 1
            if none_frame_count % (settings.FPS * 5) == 0: # Log every 5 seconds of silence
                logger.warning(f"[CAPTURE] No frames from camera for {none_frame_count / settings.FPS:.1f}s")
            continue
        ...
        if bus.enabled:
            try:
                ...
                if frame_count % 100 == 0:
                    logger.info(f"[CAPTURE] Published {frame_count} frames.")
            except Exception as e:
