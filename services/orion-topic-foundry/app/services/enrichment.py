...
def _llm_prompt(text: str, taxonomy: List[str]) -> str:
    # The `meaning`/`sentiment` shape block is generated by
    # describe_enrichment_shape() from the same constants
    # _finalize_enrichment validates against, so the two cannot drift. Before
    # this, the prompt named the keys but never their shapes, and the model
    # answered with prose strings -- 552 of 554 enriched rows in the live
    # database, which 500'd GET /segments outright. See
    # app/services/enrichment_contract.py's module docstring.
    return (
...
def _finalize_enrichment(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # is really JSON becomes the object it encodes, and genuine prose is
    # preserved under `summary` + `unstructured` rather than dropped or
    # dressed up as structure it does not have.
    result["meaning"] = coerce_meaning(result.get("meaning")) or {}
    result["sentiment"] = coerce_sentiment(result.get("sentiment")) or {}
    return result
