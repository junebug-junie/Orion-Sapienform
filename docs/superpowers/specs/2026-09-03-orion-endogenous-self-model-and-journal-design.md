...
**`self_study`** (`services/orion-cortex-exec/app/self_study.py`): a real 3-layer scanner (inspect / induce / reflect) that, when invoked, writes its result as a journal entry via `source_kind="self_study"` or `"self_reflection"` — its older direct-to-graph writeback (`orion/substrate/relational/adapters/self_study.py` → RDF) is retired. So self_study's real live output already runs *through* the journal pipeline above; it doesn't have a separate reader problem, it has the same journal→harness gap.

**Correction (adversarial pass, same day):** an earlier draft of this doc claimed self_study "has no scheduler" and cited an empty stretch of the workflow registry. That was wrong — `orion/cognition/workflows/registry.py` already registers it as `self_review`, with both `user_invocable=True` and, critically, `autonomous_invocable=True`: Orion's own autonomy/action-selection system can already choose to run it, no cron needed in principle. The real bug is one layer up: autonomy apparently never selects it in practice (independently confirmed: zero real self_study journal rows exist, ever). "Add a scheduler" was the wrong-layer fix — see the revised recommended patch below.
...
4. ~~Backfill vs. forward-only for the `orion_metacog` reducer.~~ **RESOLVED (adversarial pass, same day): forward-only, not a matter of preference.** Commit `00ae86cf6` (merged to main today) confirms 68% of all historical `orion_metacog` rows are the LLM echoing its own prompt's one-shot example instead of writing real content — 100% of `relational`-type rows, 74% of `telemetry_anomaly`, 62% of `transport`. The bug is now guarded against going forward; the accumulated history is majority contaminated. Any digest reducer must start from this fix's landing timestamp forward. Backfilling would summarize mostly-boilerplate as if it were a real pattern.
...
## Recommended next patch (revised after adversarial pass)

Smallest, thinnest, highest-leverage first slice — deliberately not "build everything above in one PR":

1. **Diagnose why autonomy never selects `self_review`, before touching scheduling at all.** It's already a valid `autonomous_invocable=True` action; zero real runs exist. Check whether it's ever even a scored candidate in the ~142k-action autonomy space, or structurally excluded/never surfaced. This determines whether the real fix is a scoring/prioritization change (thin) or actually does need a forced cadence (if guaranteed daily self-reflection is wanted regardless of what autonomy would organically choose).
2. **Fix the blank config var that leaves the `chat_stance.py:145-151` self_study read slot dead**, and **stop rendering the 100%-empty `JournalEntryIndexV1` fields.** Both small, single-file, low-risk, independently-real bugs, no dependency on the privacy question.
3. **Journal→harness wiring**, scoped to entries created after the 2026-09-03 anti-echo fix (`00ae86cf6`) — not the full history.

Step 3, and the `orion_metacog` digest reducer, are the real prize but land after open question #1 (privacy boundary) has an answer from Juniper — proposal-mode rule, not caution for its own sake, since this is the "reaches live cognition" class of change CLAUDE.md §0A gates.
