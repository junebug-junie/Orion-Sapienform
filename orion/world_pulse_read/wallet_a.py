...
WALLET_A_RETRY_NOT_BEFORE_KEY = "orion:wp_read:wallet_a:retry_not_before"
WALLET_A_REFUND_STREAK_KEY = "orion:wp_read:wallet_a:refund_streak"
...
async def debit_wallet_a(
    ...
    *,
    ...
    return await record_debit(
        ...
        retry_key=WALLET_A_RETRY_NOT_BEFORE_KEY,
        streak_key=WALLET_A_REFUND_STREAK_KEY,
        now=now,
        ttl_sec=ttl_sec,
    )
...
async def settle_wallet_a(redis, receipt: WalletDebit | None) -> None:
    """The turn reached the reader: keep the charge, reset the refund streak."""
    await settle_turn_ran(redis, receipt)


async def read_wallet_a_retry_wait(redis, *, now: datetime) -> float | None:
    """Seconds until a refunded turn may retry (None when not backing off)."""
    return await read_retry_wait(redis, WALLET_A_RETRY_NOT_BEFORE_KEY, now=now)
