...
class Node:
    ...
    def expand(self, is_print=False):
        ...
        if unc_terminal and not terminal:
            ...
            idx = idx.reshape(-1).cpu().numpy().tolist()
...
#     def search(self, prompt, golden_answer, is_print=False):
#         root = Node(prompt=prompt, state="", action=[-1], content=prompt, golden_answer=golden_answer, terminal=False, sentence_terminal=False, model=self.model, qmodel=self.qmodel, tokenizer=self.tokenizer, parent=None, b1=self.b1, b2=self.b2, policy_prob=0.5, remote_rm_url=self.remote_rm_url, action_sequence=[], max_depth=self.D)
...
#             uncertainty = (outputs["qvalue"] - (reward + self.gamma * outputs["next_qvalue"])) ** 2
#             # print(uncertainty, outputs["qvalue"])
#             unc_terminal = uncertainty < 0.015
#             action_prob += outputs["action_prob"]
#             step_count += 1
#         action_prob = action_prob / step_count 
#         # print("node action length : {}".format(step_count))
...
#         # if len(action_idx) > 1 or str(action_idx) not in [key for key in self.children.keys()]:
#             # print("\n\n\nnew node", response[0], "\n\n\nnew node \n\n\n")
#         new_node = Node(prompt=self.prompt, state=f"{self.state}{results}", action=action_idx, content=response[0], golden_answer=self.golden_answer, terminal=true_terminal, sentence_terminal=terminal, model=self.model, qmodel=self.qmodel, tokenizer=self.tokenizer, parent=self, b1=self.b1, b2=self.b2, policy_prob=action_prob, remote_rm_url=self.remote_rm_url, action_sequence=self.action_sequence+action_idx, max_depth=self.max_depth, new_depth=step_count)

#         # self.q_values[outputs["action_idx"]] = outputs["qvalue"] #- uncertainty
#         self.children[str(action_idx)] = new_node
#         # simulation value
#         # new_node.R = outputs["next_qvalue"] if not terminal else reward
#         new_node.rollout()
#         new_node.backup(new_node.gamma)
...
#             print("len node", len(node_list))
#         for node in reversed(node_list):
#             if node.value > items["maybe_best_value"] and node.depth >= maybe_depth:
#                 items["maybe_best_value"] = node.value
#                 items["maybe_response"] = node.content
#                 items["maybe_action_idx"] = node.action_sequence
#                 # items["true_term"] = False
#             if node.terminal and node.value > items["best_value"]:
#                 items["best_value"] = node.value
#                 items["response"] = node.content
#                 items["action_idx"] = node.action_sequence
#                 depth = node.depth
...
# class Node:
#     def __init__(self, prompt, state, action, content, golden_answer, terminal, sentence_terminal, model, qmodel, tokenizer, parent=None, b1=5, b2=4, policy_prob=0.5, remote_rm_url=None, action_sequence=[], max_depth=64):
#         # self.id = time.time()
#         self.prompt = prompt
#         self.state = state
#         self.action = action
...
#                 best_node = child
#                 # if is_print:
#                 #     print("select", self.depth, child.q_values.get(action, 0), transition_prob, np.sqrt(self.visits) / (child.visits + 1))
#         return best_node
...
#         # if is_print:
#         #     print(outputs["next_token"], self.tokenizer.eos_token_id)
#         #     if terminal:
#         #         print(terminal)

#         # print(terminal, self.sentence_terminal, "terminal")

#         response = self.tokenizer.batch_decode(inputs["input_ids"], skip_special_tokens=True)
#         # prompts_length = len(self.prompt) + len(self.state)
#         prompts_length = len(self.content)
#         results = response[0][prompts_length:]
#         # print(len(results))
#         # results = response[0]
#         if terminal:
...
#             # print(reward, self.depth, "reward")
#         else:
#             reward = 0.

#         uncertainty = (outputs["qvalue"] - (reward + self.gamma * outputs["next_qvalue"])) ** 2
#         # print(uncertainty, outputs["qvalue"])
#         unc_terminal = uncertainty < 0.002
#         true_terminal = terminal or unc_terminal or self.depth >= self.max_depth
#         if outputs["action_idx"] not in self.children:
#             # print("\n\n\nnew node", response[0], "\n\n\nnew node \n\n\n")
#             new_node = Node(prompt=self.prompt, state=f"{self.state}{results}", action=outputs["action_idx"], content=response[0], golden_answer=self.golden_answer, terminal=true_terminal, sentence_terminal=terminal, model=self.model, qmodel=self.qmodel, tokenizer=self.tokenizer, parent=self, b1=self.b1, b2=self.b2, policy_prob=outputs["action_prob"], remote_rm_url=self.remote_rm_url, action_sequence=self.action_sequence+[outputs["action_idx"]], max_depth=self.max_depth)
...
#                 items["maybe_action_idx"] = node.action_sequence
#                 # items["true_term"] = False
#             if node.terminal and node.value > items["best_value"] and node.depth >= depth:
#                 items["best_value"] = node.value
#                 items["response"] = node.content
#                 items["action_idx"] = node.action_sequence
#                 depth = node.depth
