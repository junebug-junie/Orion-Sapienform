...
from openrlhf.trainer import GRPOTrainer_Math as GRPOTrainer
...
class PromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.prompts[idx // self.n_samples_per_prompt], self.answers[idx // self.n_samples_per_prompt]
...
def train(args):
    ...
    # if args.intention_model:
    #     mark_only_param_as_trainable(actor, bias=[".layers", "lm_head", "embed_tokens", ".norm"])
    ...
    if not "dsp" in args.prompt_data:
        prompts_dataset = PromptDataset(prompts_data, tokenizer, strategy, input_template=args.input_template)
    else:
        from openrlhf.datasets.prompts_dataset import PromptMergeDataset
        prompts_dataset = PromptMergeDataset(prompts_data, tokenizer, strategy, input_template=args.input_template)
    ...
    # configure Trainer
    if args.intention_model:
        actor.model.model.deterministic = False
        trainer = GRPOTrainer(
            strategy,
            actor,
            reward_model,
    ...
    if args.intention_model:
        strategy.save_model(
            ema_model.model if args.enable_ema else actor.model,
            tokenizer,
            args.save_path,
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--intention_model", action="store_true", default=False)
