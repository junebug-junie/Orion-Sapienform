...
from .experience_maker import Experience, ExperienceWithAction
...
@dataclass
class BufferItemWithAction:
    """BufferItemWithAction is an item of experience data.
...
def split_experience_batch_with_action(experience: ExperienceWithAction) -> List[BufferItemWithAction]:
    batch_size = experience.sequences.size(0)
    batch_kwargs = [{} for _ in range(batch_size)]
    keys = (
        "sequences",
    ...
    items = [BufferItemWithAction(**kwargs) for kwargs in batch_kwargs]
    return items
...
def make_experience_batch_with_action(items: List[BufferItemWithAction]) -> ExperienceWithAction:
    kwargs = {}
    keys = (
        "sequences",
        "action_sequences",
    ...
    for key in items[0].info.keys():
        vals = torch.tensor([item.info[key] for item in items])
        kwargs["info"][key] = vals
    return ExperienceWithAction(**kwargs)
...
def remove_padding_in_sequences_with_action(items):
    for item in items:
        seq, act_seq, act_log_prob, value, ret, adv, att_mask, act_mask = (
            item.sequences,
            item.action_sequences,
            # item.policy_embeds,
            item.action_log_probs,
            item.values,
            item.returns,
        ...
        (
            ...
            seq[left_pad:right_pad],
            act_seq[left_pad:right_pad],
            # pi_emb[left_pad:right_pad],
            act_log_prob[:right_pad],
            value[:right_pad],
            ret[:right_pad],
            adv[:right_pad],
...
class IntentionReplayBuffer(ABC):
    """Naive replay buffer class. It stores experience.
    ...
    def __init__(self, sample_batch_size: int, limit: int = 0, cpu_offload: bool = True) -> None:
        ...
        self.cpu_offload = cpu_offload
        self.target_device = torch.device(f"cuda:{torch.cuda.current_device()}")
        self.items: List[BufferItemWithAction] = []

    @torch.no_grad()
    def append(self, experience: ExperienceWithAction) -> None:
        if self.cpu_offload:
            experience.to_device(torch.device("cpu"))
        items = split_experience_batch_with_action(experience)
        items = remove_padding_in_sequences_with_action(items)
        self.items.extend(items)
        if self.limit > 0:
            samples_to_remove = len(self.items) - self.limit
            if samples_to_remove > 0:
    ...
    @torch.no_grad()
    def sample(self) -> ExperienceWithAction:
        items = random.sample(self.items, self.sample_batch_size)
        experience = make_experience_batch_with_action(items)
        if self.cpu_offload:
            experience.to_device(self.target_device)
        return experience
    ...
    def __getitem__(self, idx: int) -> BufferItemWithAction:
        return self.items[idx]

    def collate_fn(self, batch) -> ExperienceWithAction:
        experience = make_experience_batch_with_action(batch)
        return experience
