...
def get_llm_for_sequence_regression(
    ...
    **kwargs,
    ...
    """
    assert (
        model_type == "critic" or model_type == "reward" or model_type == "q"
    ...
    try:
        ...
        if model_type == "reward":
            ...
            cls_class = _get_qvalue_model(base_pretrained_class, base_class, value_head_prefix, packing_samples, action_size=action_size)
        else:
    ...
    if init_lm_head:
        if dschf is not None:
            logger.info("initialize value_head for ZeRO-3 reward model training.")
            with deepspeed.zero.GatheredParameters([model.lm_head.weight], modifier_rank=0):
                if torch.distributed.get_rank() == 0:
                    model.lm_head.weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
        else:
            model.lm_head.weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
...
def _get_qvalue_model(base_pretrained_model, base_llm_model, value_head_prefix="value_head", packing_samples=False, action_size=64):
    class QModel(base_pretrained_model):
        supports_gradient_checkpointing = True
        ...
        def __init__(self, config: AutoConfig):
            ...
            self.value_head_prefix = value_head_prefix
            setattr(self, value_head_prefix, nn.Linear(config.hidden_size, action_size, bias=False))
        ...
        def forward(
            ...
            outputs = getattr(self, self.base_model_prefix)(
                input_ids, attention_mask=attention_mask, position_ids=position_ids, use_cache=use_cache, past_key_values=past_key_values if use_cache else None
            )
            last_hidden_states = outputs["last_hidden_state"]
        ...
        def reset_target(self, model):
            for param, target_param in zip(model.parameters(), self.parameters()):
                # print("copied", target_param.data.device, param.data.device)
