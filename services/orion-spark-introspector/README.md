...
Spark-introspector is the streaming component of Spark that reacts to live bus traffic and drives the φ chart.

### Turn change appraisal (v1)

Chat-turn **novelty** in Spark telemetry now comes from `spark_meta.turn_change_appraisal` (written by `orion-memory-consolidation` via `orion:chat:history:spark_meta:patch`), not from tissue φ deltas on the gateway path.

| Field | Source | Notes |
|-------|--------|-------|
| `spark_meta.turn_change_appraisal.novelty_score` | Logprob classifier (memory-consolidation) | Used when `turn_change_status=ok` |
| Telemetry `novelty` | `_novelty_from_spark_meta()` | `null` when appraisal missing or `degraded`/`skipped` — **no tissue fallback** |
| `phi` / coherence in telemetry | Tissue snapshot | Snapshot-only on chat candidate/trace paths (no `TISSUE.propagate` on hot path) |
| `turn_effect` | `turn_effect_from_appraisal` first | φ `turn_effect` blocked when appraisal is `degraded` |

Introspection prompts (`introspect_spark.j2`) use the appraisal block (baseline, shift kind, confidence) instead of `phi_before`/`phi_after` tables.

Legacy rows without `turn_change_appraisal` may still show φ-based `turn_effect`; new hub turns should not receive gateway φ stamping.

---

Spark-introspector still reacts to **semantic embeddings** from `orion-vector-host` (tissue propagate on the semantic upsert path only). Chat gateway flow:
...
1. **Gateway**
   ...
   * Action:
     ...
     * Attaches thin `spark_meta` on the result (`latest_user_message`, `trace_verb`) — **no `phi_before`/`phi_after` stamping**.
     * Publishes a **spark candidate** when `latest_user_message` is present:  
...
|----------------------------------|----------------------|--------------------|------------------------|-----------------------------|----------------------------------------------------|
| `orion:chat:history:spark_meta:patch` | `chat.history.spark_meta.patch.v1` | patch envelope | `orion-memory-consolidation` | hub / sql-writer consumers | includes `turn_change_appraisal` after each persisted turn |
