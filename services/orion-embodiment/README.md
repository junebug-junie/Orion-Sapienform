...
- **Speech length contract:** `build_speech_prompt` asks for a real in-scene answer that matches substance to the partner's latest line (greetings can stay brief; questions/plans get a few short spoken sentences). It deliberately does **not** say "exactly one short spoken line" — that wording was producing dismissive one-liners on substantive Juniper turns. `chat_quick.j2` mirrors the same length guidance when `metadata.surface == "aitown"`.
