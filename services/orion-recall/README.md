...
Dark-shipped chatturn-only read path standing in for RDF's chat-turn fetch. See `docs/superpowers/specs/2026-07-17-recall-rdf-writer-falkor-cutover-phase2-spec.md` (Phase 4) and `services/orion-meta-tags/README.md` (the write side this reads).

| Variable | Default | Notes |
| :--- | :--- | :--- |
| `RECALL_FALKOR_IN_CHAT` | `false` | When `true`, `app/storage/falkor_chat_adapter.py::fetch_falkor_chatturn_fragments` **swaps in** for the RDF chatturn fetch in `_query_backends` -- not additive like `RECALL_GRAPHITI_IN_CHAT` (that merges an extra rail; this replaces one, since the point of this migration is retiring RDF). Independent of `RECALL_ENABLE_RDF`. Only covers chatturn fragments (prompt/response text); `fetch_rdf_graphtri_fragments` (Claim-based) stays on Fuseki -- its whole shape assumes `Claim` nodes, which the Falkor writer replaced with `HAS_TAG`/`MENTIONS_ENTITY` edges, so it needs a redesign, not a rewrite. |
...
**Known gap: no historical backfill yet (Phase 3 unstarted).** Only chat turns tagged since ~2026-07-18 (when the observer-gate bug blocking the Falkor writer was fixed) exist in `orion_recall`'s `:ChatTurn` graph -- flipping this flag on today means near-zero chatturn recall coverage until backfill runs, not full parity with RDF. The Falkor `:ChatTurn` node is also deliberately thin (no prompt/response text -- Postgres owns that), so this is a Falkor-turn-discovery + Postgres-text-join read, not a pure single-store Cypher query; a turn with no matching `chat_history_log` row is silently dropped (nothing to quote).
