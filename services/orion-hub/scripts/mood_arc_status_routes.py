...
PHI_V2_DESIGN_DOC = REPO_ROOT / "docs" / "superpowers" / "specs" / "2026-08-21-phi-v2-design.md"
PHI_ENCODER_CLI = REPO_ROOT / "scripts" / "fit_phi_encoder.py"
...
def _design_doc_status() -> str | None:
    if not PHI_V2_DESIGN_DOC.exists():
        return None
    for line in PHI_V2_DESIGN_DOC.read_text(encoding="utf-8").splitlines():
...
async def phi_v2_inventory() -> dict[str, Any]:
    ...
    for signal_id in PHI_V2_LEGACY_SIGNAL_IDS:
        ...
        # producer_service still exist as a service directory at all.
        ...
        signals.append(
            {
                ...
                "producer_service_exists": producer_dir.is_dir(),
    ...
    return {
        ...
        "design_doc": {
            "path": str(PHI_V2_DESIGN_DOC.relative_to(REPO_ROOT)),
            "exists": PHI_V2_DESIGN_DOC.exists(),
            "status": _design_doc_status(),
        },
        "manual_cli_exists": PHI_ENCODER_CLI.exists(),
    }
