...
3. **TODO — chat-history RDF write path coverage gap (found 2026-07-17, unexplained):** Before `orion:chat` was removed from `orion-rdf-writer`'s subscriptions (2026-07-17), a live check found the Fuseki graph held only 299 `ChatTurn` nodes / 23 sessions — roughly **11-18% of actual chat volume** (Postgres `chat_message`: 2,579 rows; `chat_history_log`: 1,699 rows, same point in time). Of the turns that did make it, almost none carried the richer fields the builder supported (`model`, `intent`, `topic`, token counts never appeared as populated predicates at all; `timestamp` appeared on just 1 of 299). This gap was never root-caused — is `chat.history`/`chat.history.message.v1` traffic silently dropped somewhere upstream of rdf-writer (a `RDF_SKIP_KINDS` filter, a dead-letter queue absorbing most writes, a queue/worker capacity mismatch for chat volume specifically), or does the bus-level producer itself only emit a sliver of real chat turns onto these channels? Worth tracing independent of the RDF removal, since the same upstream mechanism (if it's a producer-side gap, not an rdf-writer-side one) could also be silently starving other consumers of `orion:chat:history:turn`/`orion:chat:history:log` (`orion-vector-writer`, `orion-vector-host`, `orion-spark-concept-induction` per `channels.yaml`) of the same missing ~85%.
