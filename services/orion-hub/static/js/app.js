...
loadDismissedIds();
  ...
  async function ensureDatasetForModelCreate() {
    const selectedId = (tsDatasetSelect?.value || "").trim();
    if (selectedId) return selectedId;
    const datasetPayload = buildDatasetSpec();
    if (!datasetPayload.name || !datasetPayload.source_table || !datasetPayload.id_column || !datasetPayload.time_column || !(datasetPayload.text_columns || []).length) {
      throw new Error("Select a dataset or fill dataset fields (name/source/id/time/text columns) to auto-create one.");
    }
    setDatasetSaveStatus("No dataset selected; creating dataset from current form...");
    const created = await topicFoundryFetch("/datasets", {
      method: "POST",
      body: JSON.stringify(datasetPayload),
    });
    const createdId = created?.dataset_id;
    if (!createdId) {
      throw new Error("Dataset auto-create did not return dataset_id.");
    }
    if (tsDatasetSelect) tsDatasetSelect.value = createdId;
    setDatasetSaveStatus(`Created dataset ${createdId}`);
    return createdId;
  }
  ...
  async function handleCreateModelClick() {
    ...
    try {
      const datasetId = await ensureDatasetForModelCreate();
