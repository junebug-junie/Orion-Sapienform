...
Asks are opened by `orion-sql-writer` (row insert + `orion:ask:opened`). The Hub does not subscribe to `orion:ask:opened`; the card polls every 60s. Needs `services/orion-sql-db/manual_migration_walkway_camera_v1.sql` applied, otherwise the routes return 503 `ask_schema_missing`. Pictures: an `image_ref` that is an http(s) URL is shown as an image; anything else (a path on the vision host) is shown as text, because the Hub has no route that serves it.
