...
## Already live (not Slice C)

**Pre-motor progress hops** (`pre_turn_appraisal`, early `association`, `thought_rpc`, `mind_enrichment`, `harness_dispatch`) ship on the Hub unified-turn path so Soft HUD can see dying appraisal / Mind / felt-state phases while the turn waits. That is Rank-1 boundary sighting — not ingress thickness.
