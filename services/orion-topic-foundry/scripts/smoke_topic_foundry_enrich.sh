...
enrich_payload=$(jq -n '{ limit: null, force: false, enricher: "heuristic" }')

curl -fsS -X POST "${TOPIC_FOUNDRY_BASE_URL}/runs/${RUN_ID}/enrich" \
  -H 'Content-Type: application/json' \
  -d "$enrich_payload" >/dev/null
...
enriched=$(echo "$run_status_resp" | jq -r '.stats.segments_enriched')
if [[ "$enriched" -lt "$MIN_ENRICHED" ]]; then
  echo "segments_enriched ${enriched} is below MIN_ENRICHED ${MIN_ENRICHED}" >&2
  exit 1
fi

segments_resp=$(curl -fsS "${TOPIC_FOUNDRY_BASE_URL}/segments?run_id=${RUN_ID}&has_enrichment=true")

has_aspects=$(echo "$segments_resp" | jq -e '.segments[] | select((.aspects | length) > 0) | .segment_id' | head -n 1)
if [[ -z "$has_aspects" ]]; then
  echo "No enriched segments with aspects found" >&2
  exit 1
fi

has_sentiment=$(echo "$segments_resp" | jq -e '.segments[] | select(.sentiment != null) | .segment_id' | head -n 1)
if [[ -z "$has_sentiment" ]]; then
  echo "No enriched segments with sentiment found" >&2
  exit 1
fi

echo "$segments_resp" | jq '.segments[0:3]'
