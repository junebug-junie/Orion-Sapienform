...
def apply_curiosity_peer_brief_retention(
    retention_days: int,
    *,
    max_batches: int | None = None,
    max_elapsed_sec: float | None = None,
) -> GrammarRetentionState:
    """Bounded retention for curiosity_peer_brief (PeerBriefV1 rows).

    Bounded from the first commit that creates the table, same rule as AttentionSchema.
    Sparse (one row per contractor-peer help response). Ages by `created_at` (write time).
    Not a grammar lane -- plain `default_engine`.
    """
    settings = get_settings()
    state = _apply_bounded_table_retention(
        engine=default_engine,
        table="curiosity_peer_brief",
        id_column="brief_id",
        retention_days=retention_days,
        batch_size=settings.grammar_events_retention_batch_size,
        max_batches=(
    ...
    _extra_retention_state["curiosity_peer_brief"] = state
    return state
