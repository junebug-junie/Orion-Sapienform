...
class BiometricsSubstrateWorker:
    ...
    def _llm_inference_tick(self) -> str | None:
        """orion-llm-gateway's own per-window call outcomes -> one llm_inference_node
        delta per serving node (orion/substrate/llm_inference_loop/). No prediction-
        error node and no per-tick side writes: the field digester is the only
        consumer, gated separately by ENABLE_LLM_INFERENCE_FIELD_DIGESTION."""
        spec = REDUCER_SPECS[5]
        events = self._store.fetch_llm_inference_grammar_events(
            limit=spec.batch_limit(self._settings),
        )
        if not events:
            return None
        ...
        def load_projection():
            loaded = self._store.load_llm_inference_projection(LLM_INFERENCE_PROJECTION_ID)
            return loaded or empty_llm_inference_projection(now=now)
