...
def batch_generate(args):
    ...
    if args.train_set:
        ...
        # prompts_data = [{"Question": d['question'], "Answer": d['answer']} for d in data][:args.max_samples]
    ...
    for prompts, rprompts, answers in pbar:
        ...
        for _ in range(N):
            ...
            action_idxs, _ = model.model.get_action_info(prob=True)
            action_idxs = [action_idxs[i].cpu().numpy().tolist() for i in range(action_idxs.shape[0])]
            ...
            # output_attention_mask = (outputs != tokenizer.pad_token_id)
            # length = output_attention_mask.sum(dim=-1) - inputs["attention_mask"].sum(dim=-1)
            # print([len(act) for act in action_idxs], length)
            # assert 0
            outputs = tokenizer.batch_decode(outputs, skip_special_tokens=True)
            for rprompt, prompt, output, answer, action_idx in zip(rprompts, prompts, outputs, answers, action_idxs):
                output = output[len(prompt) :]
                output_dataset.append({"instruction": rprompt, "output": output, "answer": answer, "action_idx": []})
    ...
    import json
    b = json.dumps(output_dataset, indent=2)
    f2 = open(os.path.join(args.output_path, "output_rank_{}_500.json".format(str(strategy.get_rank()))), 'w')
    f2.write(b)
    f2.close()
    ...
    if strategy.is_rank_0():
        ...
        files = [os.path.join(args.output_path, "output_rank_{}_500.json".format(str(rank))) for rank in range(world_size)]
        for file in files:
        ...
        b = json.dumps(output_dataset, indent=2)
        f2 = open(os.path.join(args.output_path, "output_drop.json"), 'w')
        f2.write(b)
        f2.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_set", action="store_true", default=False, help="HF tokenizer apply_chat_template"
    )
