...
def test_stream_stall_timeout_defaults_below_turn_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("HARNESS_FCC_STREAM_STALL_TIMEOUT_SEC", raising=False)
    assert motor._stream_stall_timeout_sec(900.0) == motor.DEFAULT_STREAM_STALL_TIMEOUT_SEC


def test_stream_stall_timeout_can_be_disabled_to_turn_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("HARNESS_FCC_STREAM_STALL_TIMEOUT_SEC", "0")
    assert motor._stream_stall_timeout_sec(900.0) == 900.0


@pytest.mark.asyncio
async def test_run_fcc_turn_fails_fast_on_stalled_stream(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A message that never completes must not hang for the whole turn budget.

    Reproduces the live incident (corr=59915360, 2026-07-12): a two-word
    greeting streamed 540KB from the upstream model with zero stream-json
    output before the whole-turn timeout would have fired. Before this cap,
    readline() waited up to `timeout_sec` *per line*, so this looked identical
    to a healthy turn until the full 900s budget ran out.
    """
    proc = _FakeProc(
    ...
    monkeypatch.setenv("HARNESS_FCC_STREAM_STALL_TIMEOUT_SEC", "0.01")
    ...
    assert [ev["type"] for ev in events] == ["step", "error"]
    assert events[-1]["error_code"] == "fcc_stream_stalled"
    assert events[-1]["steps_seen"] == 1
...
async def test_run_fcc_turn_whole_turn_timeout_wins_near_deadline(
    ...
    """Near the whole-turn deadline, fcc_timeout fires, not fcc_stream_stalled."""
    proc = _FakeProc([], block_after_stdout=True)
    ...
    monkeypatch.setattr(motor, "load_fcc_env", _fake_fcc_env)
    monkeypatch.setattr(motor, "_maybe_render_mcp_config", lambda **k: None)
    # Stall cap far larger than the turn budget: the turn deadline must win.
    monkeypatch.setenv("HARNESS_FCC_STREAM_STALL_TIMEOUT_SEC", "3600")

    events = []
    async for ev in motor.run_fcc_turn(
        prompt="hello",
        fcc_model_label="MODEL_HAIKU",
        correlation_id="corr-turn-timeout",
        workspace="/tmp",
        fcc_server_url="http://127.0.0.1:8082",
        auth_token="tok",
        claude_bin="claude",
    ...
    assert events[-1]["error_code"] == "fcc_timeout"
    assert proc.killed is True
...
async def test_run_fcc_turn_adds_mcp_config_when_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # bypassPermissions and left Orion with no way to run `gh pr create`
    # (the github MCP server is read-only, so no create_pull_request either).
    # See extend_mcp_argv()'s docstring.
