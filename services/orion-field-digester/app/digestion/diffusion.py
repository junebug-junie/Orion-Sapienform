...
def apply_diffusion(state: FieldStateV1, *, diffusion_rate: float) -> None:
    ...
    1. Multiple source channels legitimately feed the same target channel by
       ...
       execution_friction AND failure_pressure onto "reliability_pressure" --
       orchestration capability is meant to reflect either stressor). These were SUMMED
    ...
    into "confidence"/"available_capacity" (capability:transport did, via
    delivery_confidence/stream_backlog_health, until those were retired
    2026-09-25) AND a "pressure" target, whose derived formula below would otherwise
    ...
    the then-live stream_backlog_pressure was idle (0.0) in observed traffic,
    making both formulas agree by coincidence.
    """
