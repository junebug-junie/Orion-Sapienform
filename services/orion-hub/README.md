...
**Investigation v2 (PR1 plumbing):** When `CONTEXT_EXEC_INVESTIGATION_V2_ENABLED=true`, Hub Agent lane sends neutral `mode=investigation_v2` with `permissions` derived from the selected compute lane (`context_exec_permissions_for_llm_profile`) instead of inferring repo access from prompt keywords. Default is `false` (legacy keyword mode inference preserved). Context-exec returns a skeleton artifact until the evidence sweep lands in a later PR.
