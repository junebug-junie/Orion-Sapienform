...
This project has a knowledge graph at `graphify-out/`, an ignored directory symlink into `/mnt/storage-warm/orion-graphify`. Generated graphs, manifests, reports, archives and notes are local data, never Git/LFS artifacts. Run `scripts/install_graphify_local.sh` once per host (again after a Graphify upgrade). New worktrees bootstrap with `python3 scripts/graphify_storage.py init`; the CLI and `scripts/new_worktree.sh` also initialize it. See `docs/graphify-local-storage.md` for recovery and explicit publication to service readers. Graph byte/node ceilings are removed; accidental-loss checks remain.

Rules:
- For codebase questions, first run `graphify query "<question>"` when graphify-out/graph.json exists. Use `graphify path "<A>" "<B>"` for relationships and `graphify explain "<concept>"` for focused concepts. These return a scoped subgraph, usually much smaller than GRAPH_REPORT.md or raw grep output.
- If graphify-out/wiki/index.md exists, use it for broad navigation instead of raw source browsing.
- Read graphify-out/GRAPH_REPORT.md only for broad architecture review or when query/path/explain do not surface enough context.
- On historical branches still tracking graphs, a `pre-commit` gate backstops this: if `graphify-out/graph.json` is staged and its node count dropped more than 10% versus HEAD, the commit is refused (`scripts/check_graph_node_loss.py`, installed by `scripts/install_git_safety_hooks.sh`). The wrapper guards the *command*; this guards the *artifact*, so it fires however the damage happened -- a bare `graphify update`, another tool, a human's terminal, an agent not running under Claude Code, or a bypassed wrapper. That matters because the 2026-07-14 incident was committed and merged to main before anyone caught it, and as of 2026-08-14 the destructive update still recurs several times a day. Threshold override: `GRAPHIFY_COMMIT_MAX_NODE_LOSS_PCT`. Escape hatch for a genuine full re-extraction, set consciously per command: `ORION_ALLOW_GRAPH_SHRINK=1 git commit ...`. Verified against the real 43MB artifact: gutting it 28306 -> 2475 nodes is refused at 91.26% loss, and a commit that does not touch the graph is unaffected (whole pre-commit runs in 0.40s). None of this fixes the underlying graphify bug, which is still unroot-caused -- it contains the damage.
- After modifying code, run `scripts/safe_graphify_update.sh` (NOT `graphify update .` directly) to keep the graph current (AST-only, no API cost). A real incident (2026-07-14) found `graphify update .` can silently shrink the graph by ~95% while its own log output claims to have "kept" the old nodes -- that got committed and merged to main before being caught. The wrapper compares node count before/after and auto-restores if the drop exceeds a threshold (default 10%, override with `GRAPHIFY_UPDATE_MAX_NODE_LOSS_PCT`), so a destructive run restores the previous local bundle. **ROOT-CAUSED 2026-09-06 (PR: fix/graphify-scan-scope).** graphify 0.9.15 applies a nested `.gitignore`'s *unanchored* patterns to the entire remaining directory walk, not just that file's subtree. `services/orion-hub/tests/e2e/artifacts/.gitignore` held a bare `*`, so every directory os.walk visited after it was pruned, the scanner reported **0 code files at repo root** (4,433 once fixed), and the incremental path's AST-ownership rule evicted every code node it no longer 'owned' -- while its own log said "kept 26,564 node(s)". The ignore is now the anchored `/*` form (same meaning to git), `tests/test_gitignore_no_bare_catchall.py` refuses any tracked bare catch-all, and `scripts/check_graphify_scan_scope.py` (run by the wrapper before every refresh) refuses a hollow scan outright instead of catching the wipe after the fact. First clean refresh: 28,306 -> 74,860 nodes. If the wrapper still refuses, read its message -- it now names which layer tripped.
