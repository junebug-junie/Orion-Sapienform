...
> (`_should_auto_route()` in `services/orion-cortex-orch/app/main.py`). Every
> real producer in the map below hardcodes its `mode` (`brain`/`agent`) in
> its own source, confirmed by reading each one directly; live container
> logs across 22 real chat turns the same day showed
> `auto_depth_gate ... should_auto=False reason=intent_none` on every single
> one, with zero occurrences of `should_auto=True` -- so
> `DecisionRouter.route()` is not actually invoked in production. The
> self-modification target it fed (`routing_threshold_patch`) was retired
> 2026-09-05. Full trail (the exact log lines, the source read for each
> producer) is in this change's PR description, not reproduced here.
...
- Full verb YAML inventory (36, `chat_deep_graph` retired 2026-07-21 alongside the graphtri/deep.graph recall subsystem): `active`, `analyze_text`, `assess_risk`, `auto_route`, `chat_general`, `concept_induction`, `counterfactual`, `daily_metacog_v1`, `daily_pulse_v1`, `dream_preprocess`, `dream_simple`, `evaluate`, `extract_facts`, `goal_formulate`, `introspect`, `introspect_spark`, `log_collapse_mirror`, `log_orion_metacognition`, `pattern_detect`, `perceive_caption_frame`, `perceive_detect_open_vocab`, `perceive_embed_image`, `perceive_retina_fast`, `perceive_vision_events`, `perceive_vision_memory`, `plan_action`, `rdf_build`, `recall`, `reflect`, `search_web`, `self_critique`, `simulate`, `story_weave`, `summarize_context`, `tag_enrich`, `triage`.
