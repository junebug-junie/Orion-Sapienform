...
## Rank-aggregation math extracted to a shared module, 2026-07-31

`BordaResult`, `scorer_top1`, `_borda_points_for_scorer`, and `aggregate_borda`
moved to `orion.attention.rank_aggregation` (pure, no domain-specific
dependencies) and are re-imported/re-exported here unchanged -- this
module's own behavior and public names are byte-identical, only the
implementation's home moved. Reason: `orion.substrate.attention.salience`
(chat-level/open-loop attention, a genuinely different subsystem) became a
second real consumer of this exact Borda machinery -- replacing that
module's own hand-picked `SEED_WEIGHTS` linear blend with a rank-aggregation
of `evidence_strength`/`evidence_breadth`, the same GWT/Society-of-Mind
theory anchor this module already proved out for Layer 5. This does NOT
change this module's own "shadow candidate, not wired to any live consumer"
status for Layer 5 field attention -- see the non-goals above, still
accurate. See `orion/sentience_striving_program/README.md`'s 2026-07-31
...
__all__ = [
    "PREDICTION_ERROR_TARGET_PREFIX",
