...
logger = logging.getLogger("orion.attention.runtime.health")

_SOURCE_SERVICE = "orion-attention-runtime"
...
def run_checks(store: AttentionRuntimeStore, settings: Settings) -> list[HealthCheck]:
    checks: list[HealthCheck] = []

    retention = settings.attention_frame_retention_hours
    if retention > 0:
        age_hours = store.attention_frame_oldest_age_hours()
        stall_limit = retention * settings.attention_frame_stall_multiplier
        stalled = age_hours is not None and age_hours > stall_limit
        checks.append(
            _check(
                "attention_frame_prune_stalled",
                healthy=not stalled,
                severity="error",
                message_if_unhealthy=(
                    f"substrate_attention_frames oldest row is {age_hours:.1f}h old, exceeding "
                    f"{stall_limit:.1f}h ({settings.attention_frame_stall_multiplier}x the "
...
class HealthMonitor:
    ...
    def __init__(self, store: AttentionRuntimeStore, settings: Settings) -> None:
        self._store = store
    ...
    def _has_open_alert(self, reason: str) -> bool:
        ...
        try:
            ...
            logger.exception("attention_runtime_health_pending_lookup_failed reason=%s", reason)
    ...
    def _publish(self, check: HealthCheck, *, recovered: bool) -> bool:
        if recovered:
            message = f"[Orion attention-runtime] recovered: {check.key}"
            severity: Severity = "info"
        else:
            message = f"[Orion attention-runtime] {check.message}"
        ...
        try:
            result = self._client.attention_request(
                ...
                context={
                    ...
                    "reason": check.key,
                    "event_kind": "orion.attention_runtime.health.attention.v1",
                    "correlation_id": str(uuid4()),
                },
            )
            return bool(getattr(result, "ok", False))
        except Exception:
            logger.exception("attention_runtime_health_attention_publish_failed key=%s", check.key)
            return False
