...
- Before running semantic (LLM) extraction (`/graphify` Part B, on docs/papers/images) at repo scope, run `python3 scripts/graphify_seed_semantic_cache.py` from repo root FIRST. graphify's semantic pass skips a file only if it has a matching entry in the local, unversioned cache at `graphify-out/cache/semantic/`, keyed by content hash (`graphify.cache.check_semantic_cache`). That cache lives in a fresh worktree/clone only if seeded — without it, a full-repo semantic run will re-dispatch subagents to re-extract every document/paper/image file already represented in `graph.json` (362 files as of 2026-07-16), including the whole `docs/` corpus (PR #1074). Not destructive (`build_merge()`'s dedup-by-id keeps the existing node and drops the wasted re-extraction), just a real token/time cost worth skipping. The seed script reconstructs cache entries directly from `graph.json`'s existing nodes/edges/hyperedges, so it needs no LLM and costs nothing to run. After any future semantic extraction merge, re-run it and commit the updated `graphify-out/cache/semantic/` directory alongside `graph.json` so the next fresh worktree stays correctly seeded too — the cache is checked into git specifically so this works with zero setup from a clean clone, not just an already-primed local machine.
