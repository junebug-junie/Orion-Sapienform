"""Regression coverage for ollama-host's bus-native SystemHealthV1 heartbeat wiring.

Part of the wider service-heartbeat rollout following pilot-5 (PR #1350), see
docs/superpowers/specs/2026-07-24-service-heartbeat-node-telemetry-design.md.
orion-ollama-host had no bus connection at all before this patch (confirmed via
`grep -rl "BaseChassis\\|SystemHealthV1"` returning nothing for this service) -- its `main()`
was a purely synchronous subprocess launcher (`ollama serve` + blocking model pull). This test
does not launch a real subprocess or open a real bus connection -- it verifies (a)
...
import app.main as ollama_main
from app.settings import settings
from orion.core.bus.bus_service_chassis import HeartbeatOnly


def test_build_heartbeat_chassis_uses_ollama_settings() -> None:
    chassis = ollama_main.build_heartbeat_chassis()
...
async def test_main_async_starts_and_stops_heartbeat_chassis(monkeypatch) -> None:
    ...
    monkeypatch.setattr(ollama_main, "build_heartbeat_chassis", lambda: fake_chassis)
    monkeypatch.setattr(ollama_main, "run_ollama_server_blocking", lambda: None)

    await ollama_main._main_async()
...
async def test_main_async_survives_heartbeat_start_failure(monkeypatch) -> None:
    """Heartbeat startup failure must not prevent the real ollama launch from running."""
    fake_chassis = AsyncMock(spec=HeartbeatOnly)
    fake_chassis.start_background.side_effect = RuntimeError("bus unreachable")
    monkeypatch.setattr(ollama_main, "build_heartbeat_chassis", lambda: fake_chassis)

    launched = []
    monkeypatch.setattr(ollama_main, "run_ollama_server_blocking", lambda: launched.append(True))
...
async def test_main_async_stops_heartbeat_even_if_server_launch_raises(monkeypatch) -> None:
    ...
    def _boom():
        raise RuntimeError("ollama serve crashed")

    monkeypatch.setattr(ollama_main, "run_ollama_server_blocking", _boom)

    with pytest.raises(RuntimeError, match="ollama serve crashed"):
        await ollama_main._main_async()
