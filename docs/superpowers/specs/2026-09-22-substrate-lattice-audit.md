...
| Item | Verdict |
|------|---------|
| Dead `capability:transport → capability:orchestration` edge on `stream_backlog_pressure` | **Closed 2026-09-25: deleted** (`chore/transport-lattice-semantics`). Gate run on 72h / 121,114 ticks: the `pressure → pressure` rewire wins orchestration on 0.63% of ticks (mean 0.2088 → 0.2089), adds nothing to the arena (transport `pressure` already reaches `resource_pressure` directly), and has no theory anchor (bus-timing anomaly fraction counts bursts as well as stalls). Leftover census mapping on `node:athena → orchestration` and the `biometrics_lattice.yaml` alias deleted too; `tests/test_field_topology_edges.py` (static CI) refuses a cap-cap edge on an unwritten channel. |
| L7 transport templates with empty `dimensions`, firing ~1,200/h | **Closed 2026-09-25: three deleted** (`inspect_bus_channel_catalog`, `summarize_transport_contract_drift`, `watch_transport_backpressure`). 24h live: ~1,210 candidates/h, 27% of prepared dispatches, present on ~99% of warranted ticks. No transport-specific dimension exists to give them. Replay (1,560 warranted ticks): freed slots go to templates with a declared signal. `inspect_transport_status` kept, `reliability_pressure` semantics untouched. |
| L11 `transport_contract_drift_loop` threshold 0.70 vs observed max 0.018–0.022 | **Closed 2026-09-25: deleted.** Worse than a high bar: the motif reads `dominant_channels` on the capability attention target, and capability targets always carry `{}` (2,883/2,883 live frames), so it could not fire at any threshold. |
...
| `transport_healthy_idle` motif keyed on retired `stream_backlog_*` | **Closed 2026-09-25: deleted.** Same empty `dominant_channels`, read as 0.0, so it reported "healthy idle" whenever transport was attended at all (fired once, 2026-09-25 04:00, one fabricated `transport_stable` expectation). No consumer reads its expectation. |
