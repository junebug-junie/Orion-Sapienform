...
| `v4` (current) | `--min-generated-at 2026-08-30T00:00:00Z --held-out-blocks 5`, 37 channels (phi-v2 audit's dense-enough signals added) | 136,979 / 3.4 days | 0.415 (CI 0.396-0.436) | 0.660 | n/a — current live model. First run promoted through `cmd_promote`'s real `active.json` mechanism (`v1`-`v3` were config-documented, not formally promoted). See `orion/mood_arc/README.md`'s v4 section for the full methodology story, including a real regime-shift finding in the held-out split that a naive retrain would have missed |
