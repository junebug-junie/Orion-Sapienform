...
from openrlhf.models import Actor, GPTLMLoss, ValueLoss
...
from .ppo_utils import IntentionGrpoExperienceMaker as NaiveExperienceMaker
from .ppo_utils import ExperienceWithAction as Experience
from .ppo_utils import IntentionReplayBuffer as NaiveReplayBuffer
...
class GRPOTrainer(ABC):
    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, rand_answers in self.prompts_dataloader:
                rand_prompts = [rand_prompts[0] for _ in range(args.group_size)]
                rand_answers = [rand_answers[0] for _ in range(args.group_size)]
                experience = self.experience_maker.make_experience(rand_prompts, rand_answers, **self.generate_kwargs)
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        action_log_probs, _ = self.actor(
            experience.sequences, num_actions, attention_mask=experience.attention_mask, return_output=True, act_mode=True, action_sequences=experience.action_sequences, # policy_embeds=experience.policy_embeds,
        )
        ...
        if self.pretrain_dataloader is not None:
            ...
            output = self.actor(inputs, attention_mask=attention_mask, return_output=True, act_mode=True)
            ptx_log_probs = output
