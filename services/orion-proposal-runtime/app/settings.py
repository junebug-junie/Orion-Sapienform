...
class Settings(BaseSettings):
    ...
    # Bus-native SystemHealthV1 heartbeat (orion:system:health). Proposal-runtime had no bus
    ...
    # competing as a first-class arena candidate (orion/metacog/proposal.py).
    # Default OFF, and it ships off -- every trigger/producer in this arc flips
    # on only after a real non-degenerate artifact is observed post-deploy, not
    # because the flag exists. Candidates carry operator_review unconditionally,
    # so even enabled this cannot auto-dispatch.
    metacog_hop_propose_enabled: bool = Field(
        False, alias="ORION_METACOG_HOP_PROPOSE_ENABLED"
    )
