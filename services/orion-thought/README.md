...
| `ORION_MEMORY_CRYSTALLIZATION_CONTEXT_MAX_AGE_SEC` | `259200` | How stale the latest active crystallization can be before it's treated as absent (3 days -- NOT self-study's 6h; a crystallized memory doesn't go stale on that clock, see settings.py's own comment for the live bug this default fixes) |
