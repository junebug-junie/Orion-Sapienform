...
fetch_rdf_chatturn_fragments used to order candidates with ``ORDER BY DESC(STR(?turn))`` (a
lexical sort on the ChatTurn URI) and hardcode ``ts: 0.0`` on every returned fragment. That
meant the same fixed set of historical turns always won, and scoring._compute_recency_factor
always fell back to its neutral 0.5 weight regardless of age.

rdf_builder.py (services/orion-rdf-writer/app/rdf_builder.py) already writes a real
``ORION.timestamp`` literal on ChatTurn nodes; these tests lock in that the adapter now
