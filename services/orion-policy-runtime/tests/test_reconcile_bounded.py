"""orion-policy-runtime: the `policy_pending` reconciler is bounded, with a rare read-only full sweep (2026-09-25).

The unbounded version (`UPDATE substrate_proposal_frames ... WHERE NOT policy_pending AND NOT EXISTS (...)`
...
def _store(**kw):
    ...
    store = PolicyRuntimeStore("postgresql://u:p@127.0.0.1:1/none", **kw)
    return store


def _run(store, db, **kw):
    store._engine = db
    return store.reconcile_policy_pending(**kw)
...
class TestItGuardsTheRightTables:
    def test_spec_names_the_real_marker_and_child_fk(self):
        ...
        (_, sql, _), = updates(db)
        assert sql.startswith("UPDATE substrate_proposal_frames p SET policy_pending = true")
        assert "NOT p.policy_pending" in sql
        assert "SELECT 1 FROM substrate_policy_decision_frames c WHERE c.source_proposal_frame_id = p.frame_id" in sql
...
class TestSettings:
    def test_defaults_and_env_keys(self, monkeypatch):
        for k in ("WINDOW_SEC", "FULL_SWEEP_INTERVAL_SEC", "FULL_SWEEP_HOUR_UTC"):
            monkeypatch.delenv(f"POLICY_RECONCILE_{k}", raising=False)
        s = Settings(POSTGRES_URI="postgresql://x/y")
        assert s.policy_reconcile_window_sec == 7200.0
        assert s.policy_reconcile_full_sweep_interval_sec == DAY
        assert s.policy_reconcile_full_sweep_hour_utc == 9
        monkeypatch.setenv("POLICY_RECONCILE_WINDOW_SEC", "1800")
        monkeypatch.setenv("POLICY_RECONCILE_FULL_SWEEP_HOUR_UTC", "-1")
        s = Settings(POSTGRES_URI="postgresql://x/y")
        assert s.policy_reconcile_window_sec == 1800
        assert s.policy_reconcile_full_sweep_hour_utc == -1
