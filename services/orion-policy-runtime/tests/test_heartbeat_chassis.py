"""Regression coverage for policy-runtime's bus-native SystemHealthV1 heartbeat wiring.

Part of the service-heartbeat rollout, see
docs/superpowers/specs/2026-07-24-service-heartbeat-node-telemetry-design.md.
orion-policy-runtime had no bus connection of any kind before this patch -- it polls Postgres
directly via `PolicyRuntimeWorker` (confirmed via `grep -rl "BaseChassis"` and a grep for
...
`worker` (the real PolicyRuntimeWorker, owning a Postgres engine) is mocked out here so this
...
# app.main constructs a module-level PolicyRuntimeWorker() at import time, which requires
# POSTGRES_URI (no default) and loads SUBSTRATE_POLICY_PATH from disk (default is a path
...
os.environ.setdefault(
    "SUBSTRATE_POLICY_PATH", str(REPO_ROOT / "config" / "policy" / "substrate_policy.v1.yaml")
)

import app.main as policy_main
from orion.core.bus.bus_service_chassis import HeartbeatOnly


def test_build_heartbeat_chassis_uses_policy_runtime_settings() -> None:
    from app.settings import get_settings

    chassis = policy_main.build_heartbeat_chassis()
    s = get_settings()
...
async def test_lifespan_starts_and_stops_heartbeat_chassis(monkeypatch) -> None:
    ...
    monkeypatch.setattr(policy_main, "build_heartbeat_chassis", lambda: fake_chassis)
    monkeypatch.setattr(policy_main.worker, "start", AsyncMock())
    monkeypatch.setattr(policy_main.worker, "stop", AsyncMock())

    app = FastAPI()
    async with policy_main.lifespan(app):
        assert policy_main.heartbeat_chassis is fake_chassis
        fake_chassis.start_background.assert_awaited_once()

    fake_chassis.stop.assert_awaited_once()
    assert policy_main.heartbeat_chassis is None
    policy_main.worker.stop.assert_awaited_once()


@pytest.mark.asyncio
async def test_lifespan_survives_heartbeat_start_failure(monkeypatch) -> None:
    """Heartbeat startup failure must not prevent the real policy worker from starting."""
    fake_chassis = AsyncMock(spec=HeartbeatOnly)
    ...
    async with policy_main.lifespan(app):
        assert policy_main.heartbeat_chassis is None
        policy_main.worker.start.assert_awaited_once()
