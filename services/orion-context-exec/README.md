...
**Engine selection:** `fake` (default) | `alexzhang` (opt-in) | `smolcode` for `agent_repl`. Fallback must be explicit (`CONTEXT_EXEC_RLM_FALLBACK_ENABLED=true`) and visible in `runtime_debug` (`engine_requested`, `engine_selected`, `fallback_used`, `fallback_reason`).

**Agent REPL workbench (`mode=agent_repl`):** Uses the smolagents CodeAgent loop with deterministic read-only repo navigation tools (`repo_grep`, `repo_read`, `repo_read_range`, `repo_find_files`, `repo_tree`, `repo_outline`, `repo_list`) and workspace artifact tools (`workspace_write`, `workspace_read`, `workspace_list`, `workspace_write_patch`, `workspace_write_report`) plus `patch_validate`. Default step budget is `CONTEXT_EXEC_AGENT_REPL_MAX_STEPS=32` (surfaced on `GET /health` and in `runtime_debug.agent_repl_max_steps`). The canonical repo remains read-only — no `repo_write`, no arbitrary shell, no network tools. Workspace writes are confined to the per-run workspace under `scratch/`, `outputs/`, and `patches/`. `agent_repl` runtime debug includes semantic smolagents tool counts under `runtime_debug.agent_repl_tool_counts`; live Hub traces display semantic tool IDs when a step calls a single registered workbench tool.
...
**Investigation v2:** When `CONTEXT_EXEC_INVESTIGATION_V2_ENABLED=true`, requests use neutral `mode=investigation_v2` with profile-derived permissions (Hub compute lane / agent compat `response_profile`) and an **`answer_contract`** that gates which probes are warranted (`investigation_status`: `not_needed` | `repo_required` | `runtime_required` | `mixed_required`). Permissions are a ceiling; the contract is the warrant — conceptual/personal turns skip repo/trace sweeps. The evidence sweep runs only warranted read-only probes; reducers build `InvestigationReportV2` as an **operator sidecar** (inspectable, not user voice). A mandatory **finalize pass** (`CONTEXT_EXEC_FINALIZE_ENABLED`, default `true`) renders user-visible `RenderedAnswer` text via shared cognition finalize templates (`finalize_response_prompt.j2` / `render_from_findings_finalize_response.j2`). Hub Agent lane surfaces `final_text` as chat and keeps `InvestigationReportV2` in `metadata.context_exec`. Operator synthesis (`CONTEXT_EXEC_AGENT_SYNTHESIS_*`) polishes the operator summary only — it does not replace user discourse.
...
**Run ledger (forensic bundles):** When `CONTEXT_EXEC_RUN_LEDGER_ENABLED=true` (default), every completed run writes an immutable evidence bundle to `{CONTEXT_EXEC_RUN_ROOT}/{run_id}/` (e.g. `/var/lib/orion/context-exec/runs/ctxrun_abc123/`): `manifest.json` (`schema=orion.context_exec.run_ledger.v1`), `run.json` (full serialized `ContextExecRunV1`), `final.md`, and — when present — `artifact.json`, `runtime_debug.json`, `verb_trace.json`, and the redacted `request.json`. Writes are atomic (`*.tmp` → `fsync` → `os.replace`) and persistence is **fail-open**: a ledger write failure logs a warning and never fails the run. Secret-named keys (`token`, `secret`, `password`, `authorization`, `api_key`, `apikey`, `key`) are redacted to `[REDACTED]` across all persisted payloads (best-effort, key-name based — free text is not scrubbed). Ledger persistence does not mutate workspaces or the canonical repo.

**Workspace manager:** Context-exec allocates per-run workspaces under `/var/lib/orion/context-exec/workspaces/{run_id}` (host: `/mnt/rlm-nvme/context-exec/workspaces/{run_id}`). These are disposable raccoon cages. The canonical repo remains mounted read-only at `/repo`. Patch-producing or REPL agents must write scratch files, outputs, and diffs into the workspace, not into `/repo`. Each workspace contains `manifest.json`, `scratch/`, `outputs/`, `patches/`, and `repo/` (empty placeholder by default). Workspace allocation is enabled by default (`CONTEXT_EXEC_WORKSPACE_ENABLED=true`) and fail-open: allocation errors log a warning and never fail the run. Optional repo materialization (`CONTEXT_EXEC_WORKSPACE_MATERIALIZE_REPO=false` by default) copies a bounded subset of `/repo` into `workspace/repo/` with a byte cap (`CONTEXT_EXEC_WORKSPACE_COPY_MAX_BYTES`). `GET /health` reports workspace status under `storage.workspace`.
