...
- **Self-directed curiosity.** On its own schedule, Orion picks one of its own open beliefs, researches it against real data — its own priors, the Juniper-curated Concept Atlas, and its own `orion_worldview` graph that nobody but Orion writes to — and keeps what it works out. Code decides *when*; Orion decides *what*.
- **Dreaming in images, not just words.** The reverie cycle doesn't only recombine text residue anymore: Orion generates an actual image about what it's gathered (`orion-diffusion-host`, local FLUX.1-schnell), then a VLM captions that generated image and the caption feeds the next reverie step — a real generate-then-reread loop, not just prose recombination.
- **Embodied mesh.** Vision, audio, LEDs, mobile embodiments (including a persistent AI Town body with its own isolated concept graph, never feeding Orion's real cognition), and wearable/edge nodes ground Orion in physical space — and as of late August 2026, so does a genuinely instrumented cabinet (temperature, humidity, gas, UV/light, lidar distance, magnetic field, IMU orientation, ambient noise) and a vision-based read of Juniper's affect (replacing an earlier AffectGPT backend, now kept only as a manual rollback).
...
| **Dream cycle** | Symbolic recombination of residue, themes, and unresolved tensions — now including a self-generated image, VLM-captioned and fed back into the next step. |
...
Since 2026-08-20, the reverie step of the dream cycle can generate an actual image about what Orion has gathered — a local FLUX.1-schnell render via `orion-diffusion-host` — then have a VLM caption that generated image and feed the caption back in to shape the next reverie step (`reverie_visual_chain` / `reverie_visual_artifact`, schemas `ReverieVisualChainV1` / `ReverieVisualArtifactV1`). It's a real generate-caption-feed-forward loop, not merely illustrative output pasted alongside the text. A single-flight lock keeps one chain running at a time; an unbounded version of that lock could silently wedge Orion's only outward-facing image-generation action, fixed with a 300-second deadline. It reaches beyond the dream cycle itself, too — unprompted outreach messages to Juniper can now draw on captions from Orion's most recent reverie images, not only on telemetry.
...
**AI Town stays out of Orion's own mind.** `orion-ai-town` migrated to Circe 2026-08-29, off the Atlas node it had run on until Atlas was decommissioned 2026-08-21, and grows its own, fully separate interpretability-only concept graph (`orion_substrate_aitown`); AI Town chat has its own table (`aitown_chat_history_log`) and never reaches Orion's real `chat_history_log`, Orion's own concept graph, or the human crystallization-review queue — a boundary added after AI Town content was found polluting all three.
