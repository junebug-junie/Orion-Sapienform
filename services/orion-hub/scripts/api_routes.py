...
async def api_chat(
    ...
    if text and getattr(bus, "enabled", False) and not no_write and not workflow_metadata_only:
        try:
            ...
            # 2026-07-28: spark.candidate publish removed (spark-introspector
            # retirement). The chat.turn publish above already feeds
...
def _mutation_signal_intake_report(
    *,
    ...
    if store_attrition.get("source") == "telemetry_override":
        ...
        reason = "store_empty"
    elif store_matched == 0:
        reason = "surface_filter_rejected_all"
    elif before_zone_filter == 0:
        # Surface filter matched rows, but none survived the store's own limit
        # slice. Distinct from the zone case: widening the zone set fixes
        # nothing here.
        reason = "limit_sliced_all"
    else:
        reason = "zone_filter_rejected_all"

    starved = reason in {"surface_filter_rejected_all", "zone_filter_rejected_all", "limit_sliced_all"}
...
@router.get("/causal-geometry")
async def causal_geometry_page() -> HTMLResponse:
    from .main import TEMPLATES_DIR, build_hub_ui_asset_version

    template = (TEMPLATES_DIR / "causal_geometry.html").read_text(encoding="utf-8")
    rendered = template.replace("{{HUB_UI_ASSET_VERSION}}", build_hub_ui_asset_version())
    return HTMLResponse(
        content=rendered,
        status_code=200,
