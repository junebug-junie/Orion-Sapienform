"""orion-execution-dispatch-runtime: the `dispatch_pending` reconciler is bounded, with a rare read-only full sweep (2026-09-25).

The unbounded version (`UPDATE substrate_policy_decision_frames ... WHERE NOT dispatch_pending AND NOT EXISTS (...)`
...
def _store(**kw):
    store = ExecutionDispatchRuntimeStore("postgresql://u:p@127.0.0.1:1/none", **kw)
    return store


def _run(store, db, **kw):
    store._engine = db
    return store.reconcile_dispatch_pending(**kw)
...
class TestItGuardsTheRightTables:
    def test_spec_names_the_real_marker_and_child_fk(self):
        ...
        (_, sql, _), = updates(db)
        assert sql.startswith("UPDATE substrate_policy_decision_frames p SET dispatch_pending = true")
        assert "NOT p.dispatch_pending" in sql
        assert "SELECT 1 FROM substrate_execution_dispatch_frames c WHERE c.source_policy_frame_id = p.frame_id" in sql
...
class TestSettings:
    def test_defaults_and_env_keys(self, monkeypatch):
        for k in ("WINDOW_SEC", "FULL_SWEEP_INTERVAL_SEC", "FULL_SWEEP_HOUR_UTC"):
            monkeypatch.delenv(f"DISPATCH_RECONCILE_{k}", raising=False)
        s = Settings(POSTGRES_URI="postgresql://x/y")
        assert s.dispatch_reconcile_window_sec == 7200.0
        assert s.dispatch_reconcile_full_sweep_interval_sec == DAY
        assert s.dispatch_reconcile_full_sweep_hour_utc == 9
        monkeypatch.setenv("DISPATCH_RECONCILE_WINDOW_SEC", "1800")
        monkeypatch.setenv("DISPATCH_RECONCILE_FULL_SWEEP_HOUR_UTC", "-1")
        s = Settings(POSTGRES_URI="postgresql://x/y")
        assert s.dispatch_reconcile_window_sec == 1800
        assert s.dispatch_reconcile_full_sweep_hour_utc == -1
