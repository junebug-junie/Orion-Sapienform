...
def _store_with(age_hours: float | None) -> MagicMock:
    store = MagicMock()
    store.self_state_oldest_age_hours.return_value = age_hours
    return store
...
def test_run_checks_calls_self_state_oldest_age_hours_once():
    store = _store_with(10.0)

    run_checks(store, _settings())

    store.self_state_oldest_age_hours.assert_called_once_with()


def test_run_checks_flags_stalled_self_state_prune():
    store = _store_with(200.0)  # > 72 * 1.5

    checks = run_checks(store, _settings())

    stalled = next(c for c in checks if c.key == "self_state_prune_stalled")
...
def test_run_checks_healthy_when_table_empty():
...
def test_health_monitor_alerts_only_on_unhealthy_transition_not_every_tick():
    ...
    with patch("app.health_monitor.NotifyClient") as client_cls:
        ...
        store.self_state_oldest_age_hours.return_value = 10.0
        monitor.run_tick()  # healthy baseline observation, no alert

        store.self_state_oldest_age_hours.return_value = 200.0
...
def test_health_monitor_suppresses_first_observation_alert_when_notify_already_has_open_item():
    ...
    with patch("app.health_monitor.NotifyClient") as client_cls, patch(
        ...
        mock_get.return_value = MagicMock(
            ...
            json=lambda: [
                {
                    "source_service": "orion-self-state-runtime",
                    "reason": "self_state_prune_stalled",
                }
            ],
        )
        monitor = HealthMonitor(store, _settings())
