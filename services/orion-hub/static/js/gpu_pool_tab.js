(function () {
  // The GPU pool tab body is a standalone iframe (/gpu-pool, served by
  // gpu_pool_routes.py) that fetches and renders its own data. app.js's tab
  // router does not know about #gpu-pool, so this script's only job is panel
  // show/hide + hash lifecycle + the iframe refresh button (same pattern as
  // hub_surface_tab.js).
  const PANEL_HASH = "#gpu-pool";

  document.addEventListener("DOMContentLoaded", () => {
    const panel = document.getElementById("gpu-pool");
    const tabButton = document.getElementById("gpuPoolTabButton");
    const panelFrame = document.getElementById("gpuPoolPanelFrame");
    const panelRefresh = document.getElementById("gpuPoolPanelRefresh");
    ...
    function activatePanel() {
      document.querySelectorAll("#appPanels section[data-panel]").forEach((section) => {
        ...
        section.classList.toggle("hidden", key !== "gpu-pool");
      });
