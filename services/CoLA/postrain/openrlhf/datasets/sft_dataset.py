...
def preprocess_data(data, input_template=None, input_key="input", output_key=None, apply_chat_template=None):
    if apply_chat_template:
        ...
        response = apply_chat_template(format_response, tokenize=False)[len(prompt) :]
        # if output_key:
...
def preprocess_data_split(data, input_template=None, input_key="input", output_key=None, apply_chat_template=None):
    ...
    # prompt_check = re.findall(patten_prompt, neg_sentence)[0]
    pos_answer = re.findall(patten_answer, pos_sentence)[0]
    # neg_answer = re.findall(patten_answer, neg_sentence)[0]
...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        self.additional_key = getattr(self.strategy.args, "additional_output_key", None)
        if self.additional_key is not None:
            output_key = self.output_key
            self.output_key = self.additional_key

            additional_processed_dataset = dataset.map(
                self.process_data, remove_columns=dataset.column_names, num_proc=num_processors
            )
            additional_processed_dataset = additional_processed_dataset.filter(lambda x: x["prompt"] is not None)

            # Store the processed data in class attributes
            self.additional_prompts = additional_processed_dataset["prompt"]
            self.additional_responses = additional_processed_dataset["response"]
            self.additional_prompt_ids_lens = additional_processed_dataset["prompt_ids_len"]

            self.prompts += self.additional_prompts
            self.responses += self.additional_prompts
            self.prompt_ids_lens += self.additional_prompt_ids_lens
...
class SFTMergeDataset(Dataset):
    ...
    def process_data(self, data):
        # prompt, response = preprocess_data_split(
        #     data,
        #     None if self.pretrain_mode else self.input_template,
        #     self.input_key,
        #     self.output_key,
